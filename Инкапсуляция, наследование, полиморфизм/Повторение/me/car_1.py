        ...
        try:
            if self.__ready_status():
                ...
                for i in range(distance):
                    ...
                    print(f'\nНепрервное время в пути {time_driving}')

                    try:
                        if time_driving >= 0.1:
                            raise MoveStop
                    except MoveStop:
                        print(f'Требуется принудительная остановка')
                        time.sleep(5)
                        part_distance = 0
                    print(f'Путь пройден')
        except (EngineIsNotRunning, DriverNotFoundError) as e:
            print(f"Машина не может начать движение, т.к. {e}")
    # /Блок отработки движения машины

    def check_average_consumption(self, fuel: int):
        if fuel / self.__mileage > 10:
            return f'Слишком большой расход топлива {fuel / self.__mileage}'
        return f'Расход топлива в пределах нормы {fuel / self.__mileage}'
...
if __name__ == '__main__':
    ...
    car.move()
    print(car.check_average_consumption(150))
    #car.move()
    # /Блок отработки движения машины
